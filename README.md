...
db_replica
변수 관리
민감한 정보는 group_vars/db/vault.yml에 정의하고 Ansible Vault를 통해 암호화하여 관리합니다.
...
복제 연결에 필수적인 repl_user만 Primary에서 선제 생성.

Primary-Replica 복제 연결 완료.

이후 모든 앱 전용 계정(app_user) 및 DB는 Primary에서만 생성하여 바이너리 로그를 통해 Replica로 자동 전파되도록 구성했습니다.
